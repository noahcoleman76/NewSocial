...
import { PrismaClient, AccountStatus, ConnectionRequestStatus, NotificationType, ReportStatus, ReportTargetType, UserRole } from '@prisma/client';
...
async function main() {
    ...
    const [childOne, childTwo, userA, userB, userC, disabledUser, disabledParent, disabledChild, deletedGhost] = await Promise.all([
        prisma.user.create({
            data: {
    ...
    await prisma.connectionRequest.createMany({
        data: [
            { senderId: userC.id, receiverId: parent.id, status: ConnectionRequestStatus.PENDING },
            { senderId: userA.id, receiverId: childTwo.id, status: ConnectionRequestStatus.PENDING },
        ],
    });
